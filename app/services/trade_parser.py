...
#
#   VS keyword splits a trade into two legs (e.g., leg1 VS leg2)
...
def parse_trade_input(input_line: str) -> list[TradeInput]:
    ...
    # =====================================================================
    if vs_index == 0:
        raise ParseError("VS found at start of input with no left leg.")
    if vs_index >= len(parts) - 1:
        raise ParseError("VS found at end of input with no right leg.")

    left_tokens = [p for p in parts[:vs_index] if p.upper() != "VS"]
    right_tokens = [p for p in parts[vs_index + 1:] if p.upper() != "VS"]

    leg1 = parse_single_leg(left_tokens)
    leg2 = parse_single_leg(right_tokens)

    leg1.volume = parsed_volume
    leg1.premium = parsed_premium
    leg2.volume = parsed_volume
    leg2.premium = parsed_premium
    leg1.suppress_premium = True
    leg2.suppress_premium = True

    stupid_mode = leg1.is_stupid or leg2.is_stupid
    opp_side = ("S" if parsed_side == "B" else "B") if not stupid_mode else parsed_side

    leg1.direction_side = parsed_side
    leg2.direction_side = opp_side

    validate_strikes(leg1)
    validate_strikes(leg2)
