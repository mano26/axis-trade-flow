...
from app.services.card_generator import generate_cards_html, build_card_data_snapshot
...
@cards_bp.route("/order/<int:order_id>/generate")
@login_required
def generate(order_id):
    ...
    try:
        ...
        flash(f"Cannot generate cards: {'; '.join(e.errors)}", "danger")
        return redirect(url_for("orders.detail", order_id=order.id))

    # Create print event
    snapshot = build_card_data_snapshot(order)
    print_event = PrintEvent(
        tenant_id=current_user.tenant_id,
        order_id=order.id,
        event_type=PrintEventType.CARD,
        printed_by_id=current_user.id,
        data_snapshot=snapshot,
    )
    db.session.add(print_event)
    audit_service.log_print_event(order, current_user.tenant_id, "card")
    db.session.commit()

    # Generate and return HTML
    html = generate_cards_html(order)
